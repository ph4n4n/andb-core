...
// Auto-execute if run directly (for npx and bin usage)
if (require.main === module) {
  const fs = require('fs');
  const path = require('path');
  const yaml = require('js-yaml');

  // 1. Try to load andb.yaml from current directory
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      console.error('❌ Error parsing andb.yaml:', e.message);
    }
  }

  // 2. Build and Parse
  const cli = module.exports.build({
    ...config,
    baseDir: process.cwd(),
    logName: 'ANDB-CLI'
  });
